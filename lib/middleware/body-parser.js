...
const parseContentType = (contentTypeHeader) => {
  if (!contentTypeHeader) { return { contentType: null } }
        ...
        dice.on('part', function (part) {
          ...
          part.on('header', function (headers) {
            ...
            if (headers['content-disposition']) {
              var disposition = contentTypeParser.parse(headers['content-disposition'][0].replace('form-data', 'form-data/text') || 'form-data/text')
            ...
            if (headers['content-type']) {
              var { contentType: partContentType } = parseContentType(headers['content-type'][0] || 'application/octet-stream')
